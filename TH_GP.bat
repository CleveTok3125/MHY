...
echo Checking if all necessary files to update the game from Patch are present...
...
for /F "usebackq delims=" %%i in ("!WorkingDirForAdmin!GenshinImpact_Data\Persistent\audio_lang_14") do (
	...
	REM Check if required TH_GP_AudioPatch_[language].txt files are present, variable will be set to true if at least one file is missing. All missing files will be listed.
	if NOT exist "!WorkingDirForAdmin!TH_GP_AudioPatch_!CurrentLanguage!.txt" (
		echo "TH_GP_AudioPatch_!CurrentLanguage!.txt" is missing.
		set FileMissing=True
		set CurrentLanguage=None
	)
)
if NOT exist "!WorkingDirForAdmin!TH_GP_AudioPatch_Common.txt" (
	echo "TH_GP_AudioPatch_Common.txt" is missing.
	set FileMissing=True
)
...
for /L %%i in (1,1,4) do (
	...
	if NOT "!CurrentLanguage!"=="None" (
		...
		for /F "usebackq delims=" %%j in ("!WorkingDirForAdmin!TH_GP_AudioPatch_!CurrentLanguage!.txt") do (
...
for /F "usebackq delims=" %%i in ("!WorkingDirForAdmin!TH_GP_AudioPatch_Common.txt") do (
...
for /L %%i in (1,1,4) do (
	...
	if NOT "!CurrentLanguage!"=="None" (
		...
		for /F "usebackq delims=" %%k in ("!WorkingDirForAdmin!TH_GP_AudioPatch_!CurrentLanguage!.txt") do (
...
for /F "usebackq delims=" %%i in ("!WorkingDirForAdmin!TH_GP_Cleanup.txt") do (
...
REM Delete source file for TH_GP_Cleanup_[language].txt and the file itself.
if exist "!WorkingDirForAdmin!deletefiles.txt" (
	del "!WorkingDirForAdmin!deletefiles.txt"
)
if exist "!WorkingDirForAdmin!TH_GP_Cleanup.txt" (
	del "!WorkingDirForAdmin!TH_GP_Cleanup.txt"
)
REM Delete source file for TH_GP_AudioPatch_[language].txt and the file itself.
if exist "!WorkingDirForAdmin!hdifffiles.txt" (
	del "!WorkingDirForAdmin!hdifffiles.txt"
)
if exist "!WorkingDirForAdmin!TH_GP_AudioPatch_Common.txt" (
	del "!WorkingDirForAdmin!TH_GP_AudioPatch_Common.txt"
)
if exist "!WorkingDirForAdmin!TH_GP_AudioPatch_Chinese.txt" (
	del "!WorkingDirForAdmin!TH_GP_AudioPatch_Chinese.txt"
)
if exist "!WorkingDirForAdmin!TH_GP_AudioPatch_English.txt" (
	del "!WorkingDirForAdmin!TH_GP_AudioPatch_English.txt"
)
if exist "!WorkingDirForAdmin!TH_GP_AudioPatch_Japanese.txt" (
	del "!WorkingDirForAdmin!TH_GP_AudioPatch_Japanese.txt"
)
if exist "!WorkingDirForAdmin!TH_GP_AudioPatch_Korean.txt" (
	del "!WorkingDirForAdmin!TH_GP_AudioPatch_Korean.txt"
)
...
if "%PatchFinished%"=="True" (
	if exist "!WorkingDirForAdmin!TH_GP.bat" (
		del "!WorkingDirForAdmin!TH_GP.bat"
	)
)